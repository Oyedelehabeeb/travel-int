...
export class FixtureTravelProvider implements TravelIntelligenceProvider {
  ...
  async comparePassports(firstPassportSlug: string, secondPassportSlug: string) {
    ...
    const isNigeriaAndGhana = nigeriaAndGhana.has('NGA') && nigeriaAndGhana.has('GHA')

    return {
      first,
      second,
      combinedScore: isNigeriaAndGhana ? 593 : Math.max(first.score, second.score),
      accessibleTogether: isNigeriaAndGhana ? 118 : Math.max(first.visaFreeCount, second.visaFreeCount),
      onlyFirst: isNigeriaAndGhana ? (first.passport.code === 'NGA' ? 4 : 31) : 0,
      onlySecond: isNigeriaAndGhana ? (second.passport.code === 'GHA' ? 31 : 4) : 0,
      both: isNigeriaAndGhana ? 83 : Math.min(first.visaFreeCount, second.visaFreeCount),
      neither: isNigeriaAndGhana ? 132 : 250 - Math.max(first.visaFreeCount, second.visaFreeCount),
    }
  }
}
